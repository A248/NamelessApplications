 ...
 */
$builderapp = $queries->getWhere('addons', array('name', '=', 'BuilderApp'));
if($builderapp[0]->enabled == 0){
	Redirect::to('/');
	die();
}
...
if(Input::exists()){
	if(Token::check(Input::get('token'))){
		...
		$queries->create('builderapp_replies', array(
			'uid' => $user->data()->id,
			'time' => date('U'),
			'content' => $content
		));
		...
		// Moderator alerts
		$mod_groups = $queries->getWhere('groups', array('builderapp', '=', 1));
		foreach($mod_groups as $mod_group){
			...
			foreach($mod_users as $individual){
				$queries->create('alerts', array(
					...
					'url' => '/mod/builderapp/?app=' . $app_id,
					'content' => str_replace('{x}', htmlspecialchars($user->data()->username), $builderapp_language['new_builder_app_submitted_alert']),
					'created' => date('U')
				));
			}
		}
...
if(!isset($completed)){
	// Has the user already submitted a Builder Application?
	$already_submitted = $queries->getWhere('builderapp_replies', array('uid', '=', $user->data()->id));
...
$page = $builderapp_language['builder_app'];
...
<html lang="en">
  <head>
    ...
    <meta name="description" content="Builder Application page for the <?php echo $sitename; ?> community">
	...
	?>
	  <div class="well">
		...
		<?php
			...
			if($completed === 2){
		?>
		<div class="alert alert-info"><?php echo $builderapp_language['builder_app_already_submitted']; ?></div>
		<?php
			}
		}
		?>
