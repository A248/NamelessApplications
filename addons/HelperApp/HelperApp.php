...
class HelperApp {
	...
	// Can the specified user view helper app?
	public function canViewHelperApp($user_id = null){
		if($user_id){
	...
	// Can the specified user accept helper app?
	public function canAcceptHelperApp($user_id = null){
		if($user_id){
			$data = $this->_db->get('users', array('id', '=', $user_id));
			if($data->count()){
				$user_group = $data->results();
